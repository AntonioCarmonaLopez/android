        ...
        if (mOp != -1) {                    // MtoDptosFragment requiere una operación válida!!
            ...
            switch (mOp) {
                case OP_CREAR:
                    ...
                    if(etClave.getText().toString().equals("") || etClave.getText().toString().length()<=6)
                        Snackbar.make(getActivity().findViewById(android.R.id.content),R.string.passwd_ko,Snackbar.LENGTH_SHORT).show();
                    break;
                case OP_EDITAR:
