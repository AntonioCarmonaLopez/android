...
public class PrefFragment extends PreferenceFragmentCompat
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        if (login != null && login.getId() == 0) { // admin
            Preference pref;
            pref = findPreference(getString(R.string.MySQL_url_key));
            if (pref != null) pref.setVisible(true);
            pref = findPreference(getString(R.string.MySQL_name_key));
            if (pref != null) pref.setVisible(true);
        }
    }
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        ...
        if (key.equals(getResources().getString(R.string.MySQL_url_key))) {
            if (login != null && login.getId() == 0) { // admin
                Toast.makeText(requireActivity(), R.string.msg_CambioBD, Toast.LENGTH_LONG).show();
            } else {
                sharedPreferences.edit().putString(key, getResources().getString(R.string.MySQL_url_default)).apply();
                Toast.makeText(requireActivity(), R.string.msg_LoginPermisos, Toast.LENGTH_LONG).show();
            }
        } else if (key.equals(getResources().getString(R.string.MySQL_name_key))) {
            if (login != null && login.getId() == 0) { // admin
                Toast.makeText(requireActivity(), R.string.msg_CambioBD, Toast.LENGTH_LONG).show();
            } else {
                sharedPreferences.edit().putString(key, getResources().getString(R.string.MySQL_name_default)).apply();
                Toast.makeText(requireActivity(), R.string.msg_LoginPermisos, Toast.LENGTH_LONG).show();
            }
        }
    }
