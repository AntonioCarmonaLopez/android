...
import com.dam.t07p02.vista.adaptadores.AdaptadorDptos;
import com.dam.t07p02.vista.adaptadores.AdaptadorIncs;
...
public class BusIncsFragment extends Fragment {
    ...
    private SwipeRefreshLayout srIncs;
    private Button btEliminar, btEditar, btCrear;

    private Departamento mLogin;

    public static final String TAG = "BusIncsFragment";
    private BusIncsFragment.BusIncsFragInterface mListener;

    public interface BusIncsFragInterface {

        void onCrearBusIncsFrag();

        void onEditarBusIncsFrag(Incidencia incidencia);

        void onEliminarBusIncsFrag(Incidencia incidencia);

        void onFiltroBusIncFrag();
    }
    ...
    public void onAttach(@NonNull Context context) {
        ...
        if (context instanceof BusIncsFragment.BusIncsFragInterface) {
            mListener = (BusIncsFragment.BusIncsFragInterface) context;
        } else {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (mLogin != null) {
            if (!incsVM.isFiltroActivo()) {
                // Inits Incs Observer
                incsVM.getIncs(mLogin).observe(this, new Observer<List<Incidencia>>() {
                    @Override
                ...
                // Inits Incs Observer
                incsVM.getIncsFilto(filtro, mLogin).observe(this, new Observer<List<Incidencia>>() {
                    @Override
                    public void onChanged(List<Incidencia> incs) {
                        mAdaptadorIncs.setDatos(incs);
                        mAdaptadorIncs.notifyDataSetChanged();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        srIncs = v.findViewById(R.id.srIncs);
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        srIncs.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            @Override
            public void onRefresh() {
                IncsViewModel incsVM = new ViewModelProvider(requireActivity()).get(IncsViewModel.class);
                incsVM.getIncs(mLogin);
                srIncs.setRefreshing(false);
            }
        });
    }
    ...
    private View.OnClickListener btFiltro_OnClickListener=new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            mListener.onFiltroBusIncFrag();
        }
    };
}
